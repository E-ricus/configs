# Niri compositor — combined NixOS enablement + full home-manager configuration.
# Includes all keybinds, layout, window rules, idle management, etc.
{ den, inputs, ... }: let
  niriNixosModule = inputs.niri.nixosModules.niri;
in {
  den.aspects.niri = {
    includes = [den.aspects.wayland];

    nixos = { pkgs, ... }: {
      imports = [niriNixosModule];
    ...
    homeManager = { config, pkgs, lib, ... }: let
      ...
      lockScript =
        if isDms
        then pkgs.writeShellScript "lock-screen" "dms ipc lock lock"
        else if isNoctalia
    ...
    in {
      ...
      programs.niri.settings = {
        ...
        # Default scale — overridden per-host via provides.to-users in host aspects
        outputs."eDP-1" = {
        ...
        overview.backdrop-color = "#26233a";
        ...
        binds = let
          ...
          launcherBind =
            if isDms then ni {action.spawn = dms "spotlight toggle";}
            else if isNoctalia then ni {action.spawn = noctalia "launcher toggle";}
            else ni {action.spawn = ["fuzzel"];};

          lockBind =
            if isDms then ni {action.spawn = dms "lock lock";}
            else if isNoctalia then ni {action.spawn = noctalia "lockScreen lock";}
            else ni {action.spawn = ["hyprlock"];};

          volumeRaiseBind =
            if isDms then { allow-when-locked = true; action.spawn = dms "audio increment 3"; }
            else if isNoctalia then { allow-when-locked = true; action.spawn = noctalia "volume increase"; }
            else { allow-when-locked = true; action.spawn = ["${volumeScript}" "raise"]; };

          volumeLowerBind =
            if isDms then { allow-when-locked = true; action.spawn = dms "audio decrement 3"; }
            else if isNoctalia then { allow-when-locked = true; action.spawn = noctalia "volume decrease"; }
            else { allow-when-locked = true; action.spawn = ["${volumeScript}" "lower"]; };

          volumeMuteBind =
            if isDms then { allow-when-locked = true; action.spawn = dms "audio mute"; }
            else if isNoctalia then { allow-when-locked = true; action.spawn = noctalia "volume muteOutput"; }
            else { allow-when-locked = true; action.spawn = ["${volumeScript}" "toggle-mute"]; };

          brightnessUpBind =
            if isDms then { allow-when-locked = true; action.spawn = dms "brightness increment 5 "; }
            else { allow-when-locked = true; action.spawn = ["${brightnessScript}" "raise"]; };

          brightnessDownBind =
            if isDms then { allow-when-locked = true; action.spawn = dms "brightness decrement 5 "; }
            else { allow-when-locked = true; action.spawn = ["${brightnessScript}" "lower"]; };
        in
          {
            "Mod+Shift+Slash" = ni {action.show-hotkey-overlay = {};};
            "Mod+Return" = ni {action.spawn = ["ghostty"];};
            "Mod+D" = launcherBind;
            "Mod+Shift+D" = ni {action.spawn = ["fuzzel"];};
            ...
            "XF86AudioMicMute" =
              if isDms then { allow-when-locked = true; action.spawn = dms "audio micmute"; }
              else { allow-when-locked = true; action.spawn-sh = "wpctl set-mute @DEFAULT_AUDIO_SOURCE@ toggle"; };

            "XF86AudioPlay" =
              if isDms then { allow-when-locked = true; action.spawn = dms "mpris playPause"; }
              else { allow-when-locked = true; action.spawn-sh = "playerctl play-pause"; };
            "XF86AudioStop" =
              if isDms then { allow-when-locked = true; action.spawn = dms "mpris stop"; }
              else { allow-when-locked = true; action.spawn-sh = "playerctl stop"; };
            "XF86AudioPrev" =
              if isDms then { allow-when-locked = true; action.spawn = dms "mpris previous"; }
              else { allow-when-locked = true; action.spawn-sh = "playerctl previous"; };
            "XF86AudioNext" =
              if isDms then { allow-when-locked = true; action.spawn = dms "mpris next"; }
              else { allow-when-locked = true; action.spawn-sh = "playerctl next"; };

            "XF86MonBrightnessUp" = brightnessUpBind;
            "XF86MonBrightnessDown" = brightnessDownBind;

            "Mod+O" = ni { repeat = false; action.toggle-overview = {}; };
            "Mod+Q" = ni { repeat = false; action.close-window = {}; };
            ...
            "Mod+WheelScrollDown" = ni { cooldown-ms = 150; action.focus-workspace-down = {}; };
            "Mod+WheelScrollUp" = ni { cooldown-ms = 150; action.focus-workspace-up = {}; };
            "Mod+Ctrl+WheelScrollDown" = ni { cooldown-ms = 150; action.move-column-to-workspace-down = {}; };
            "Mod+Ctrl+WheelScrollUp" = ni { cooldown-ms = 150; action.move-column-to-workspace-up = {}; };
            ...
            # Screenshots
            "Print" =
              if isDms then ni {action.spawn = dms "niri screenshot";}
      ...
      home.packages = with pkgs;
        [hyprlock]
        ++ lib.optionals (!isDms) [satty];

      # Idle management (only when DMS is not active)
