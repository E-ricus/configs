...
# Instead of the sequential approach (fingerprint first, then password fallback),
# this module starts the fingerprint reader in the background and immediately
# returns control so pam_unix can show the password prompt. Whichever method
# succeeds first (finger touch or password entry) wins.
#
# Upstream: https://gitlab.com/mishakmak/pam-fprint-grosshack
# Based on fprintd v1.94.2
{
  lib,
  stdenv,
  fetchFromGitLab,
