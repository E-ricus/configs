{
  ...
  options = {
    ...
    gaming-config.minecraft.enable =
      lib.mkEnableOption "enables minecraft's prismlauncher";
  };

  config = lib.mkMerge [
    (lib.mkIf config.gaming-config.enable {
      # Gaming packages
    ...
    (lib.mkIf (config.gaming-config.enable
      && config.gaming-config.minecraft.enable) {
      environment.systemPackages = with pkgs; [
        prismlauncher

        # Nvidia prism launcher
        (makeDesktopItem {
